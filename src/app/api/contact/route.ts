...
function isAllowedTurnstileHostname(
  ...
  if (
    productionTurnstileHostnames.has(
      normalizedHostname
    )
  ) {
    return true;
  ...
  return (
    ...
    developmentTurnstileHostnames.has(
      normalizedHostname
    )
  );
}
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          "O nome da empresa ou organização é muito extenso.",
      },
      { status: 400 }
    );
  }
  ...
  const text = [
    "Novo contato profissional pelo portfólio",
    "",
    `Nome: ${name}`,
    `E-mail: ${email}`,
    `Empresa ou organização: ${company || "Não informado"}`,
    `Assunto: ${subject}`,
    "",
    "Mensagem:",
    message,
          ...
          <div style="max-width:680px;margin:0 auto;border:1px solid rgba(125,255,201,.22);border-radius:24px;overflow:hidden;background:#0b1b22;">
            ...
            <div style="padding:30px;">
              <table role="presentation" style="width:100%;border-collapse:collapse;">
                <tr>
                  <td style="padding:10px 0;color:#8fb0aa;width:150px;">Nome</td>
                ...
                <tr>
                  <td style="padding:10px 0;color:#8fb0aa;">Empresa ou organização</td>
                  <td style="padding:10px 0;color:#edfdf8;font-weight:700;">${safeCompany}</td>
                </tr>
                <tr>
                  <td style="padding:10px 0;color:#8fb0aa;">Assunto</td>
                  <td style="padding:10px 0;color:#edfdf8;font-weight:700;">${safeSubject}</td>
                </tr>
              </table>
